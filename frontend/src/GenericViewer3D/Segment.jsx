...
import { isInFrustum } from './performanceUtils';
...
export function Segment({ name, onLoad, path = '/models/segments' }) {
  const basePath = path.endsWith('/') ? path.slice(0, -1) : path;
  const materials = useLoader(MTLLoader, `${basePath}/${name}.mtl`);
  const obj = useLoader(OBJLoader, `${basePath}/${name}.obj`, (loader) => {
    materials.preload();
