...
import { createHistoryState, canNavigateBack, canNavigateForward } from './historyManager';
  ...
  render() {
    if (this.state.hasError) {
      if (typeof console !== 'undefined' && console.error) {
        console.error('[GenericViewer3D] Model error:', this.state.error);
      }
      return (
        <Html center>
...
export default function Hologram({ isProcessing = false, path = '/models/segments' }) {
  ...
  const citedOrgans = useViewerStore((s) => s.citedOrgans);
  const citedOrganIndex = useViewerStore((s) => s.citedOrganIndex);
  const goToNextCitedOrgan = useViewerStore((s) => s.goToNextCitedOrgan);
  const goToPrevCitedOrgan = useViewerStore((s) => s.goToPrevCitedOrgan);
  const segmentVisibility = useViewerStore((s) => s.segmentVisibility);
  const navigationHistory = useViewerStore((s) => s.navigationHistory);
  const historyIndex = useViewerStore((s) => s.historyIndex);
  const addToHistory = useViewerStore((s) => s.addToHistory);
  const navigateHistory = useViewerStore((s) => s.navigateHistory);
  const historyPushRequest = useViewerStore((s) => s.historyPushRequest);
  ...
  const handleHistoryNavigation = useCallback((direction) => {
    ...
    if (state.segmentVisibility && typeof state.segmentVisibility.forEach === 'function') {
      state.segmentVisibility.forEach((visible, segmentName) => {
        useViewerStore.getState().setSegmentVisibility(segmentName, visible);
      });
    }

    if (state.cameraState != null) {
      useViewerStore.getState().setPendingCameraRestore(state.cameraState);
    }
  }, []);
  ...
  const saveToHistory = useCallback(() => {
    const cameraState = useViewerStore.getState().getCameraState?.() ?? null;
  ...
  return (
    <div className="w-full h-full bg-white overflow-hidden relative group">
      ...
      <div
        ...
        style={{ filter: isProcessing ? 'saturate(0.97)' : 'saturate(1)' }}
      >
      <SegmentFilterPanel />
